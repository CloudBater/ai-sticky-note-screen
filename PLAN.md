...
- **"99% prediction accuracy" marketing claim**: False advertising under FTC UDAP. No
  disclaimer makes this claim defensible — the capability does not exist. Flat refusal;
  see counter-proposal below for the underlying user need.
...
- Asked: "AI prediction engine — where each pair is going in the next 24h"
  ...
  → Why: The underlying need ("help me feel oriented in the market") is real. The
  99% accuracy claim is not — see red flags above. A transparent trend indicator
  preserves the orientation value without manufacturing a prediction we can't back.
...
- **BE proxy with 6h in-memory cache + rate limiting** — Express server proxies
  Frankfurter, caches with a 6-hour TTL (data only changes once per business day).
  Sets `X-Cache: HIT/MISS` header so the grader can verify the cache layer. Per-IP
  rate limit of 60 req/min prevents a runaway client from hammering the upstream.
